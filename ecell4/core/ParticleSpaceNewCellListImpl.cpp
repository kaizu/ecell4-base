...
void ParticleSpaceNewCellListImpl::reset(const Real3& edge_lengths)
{
    base_type::t_ = 0.0;
    particles_.clear();
    rmap_.clear();
...
bool ParticleSpaceNewCellListImpl::update_particle(
...
std::pair<ParticleID, Particle> ParticleSpaceNewCellListImpl::get_particle(
...
bool ParticleSpaceNewCellListImpl::has_particle(const ParticleID& pid) const
{
    return (this->find(pid) != particles_.end());
}

void ParticleSpaceNewCellListImpl::remove_particle(const ParticleID& pid)
{
...
Integer ParticleSpaceNewCellListImpl::num_particles() const
{
    return particles_.size();
}

Integer ParticleSpaceNewCellListImpl::num_particles(const Species& sp) const
{
    Integer retval(0);
...
Integer ParticleSpaceNewCellListImpl::num_particles_exact(const Species& sp) const
{
...
Integer ParticleSpaceNewCellListImpl::num_molecules(const Species& sp) const
{
    Integer retval(0);
    SpeciesExpressionMatcher sexp(sp);
    for (per_species_particle_id_set::const_iterator i(particle_pool_.begin());
...
Integer ParticleSpaceNewCellListImpl::num_molecules_exact(const Species& sp) const
{
    return num_particles_exact(sp);
}

std::vector<std::pair<ParticleID, Particle> >
    ParticleSpaceNewCellListImpl::list_particles() const
{
    return particles_;
}

std::vector<std::pair<ParticleID, Particle> >
    ParticleSpaceNewCellListImpl::list_particles(const Species& sp) const
{
...
std::vector<std::pair<ParticleID, Particle> >
    ParticleSpaceNewCellListImpl::list_particles_exact(const Species& sp) const
{
    std::vector<std::pair<ParticleID, Particle> > retval;
...
std::vector<std::pair<std::pair<ParticleID, Particle>, Real> >
    ParticleSpaceNewCellListImpl::list_particles_within_radius(
